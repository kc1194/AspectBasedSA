...
def runCRF(crossVal):
    if not crossVal:
        ...
        for ele in range(3):
            os.system('./../tools/CRF++-0.58/crf_learn ../src/patternFile.txt ../output/parseProcessedTrain'+str(ele)+str((ele+1)%3)+'.txt ../output/crf++_model'+str(ele)+str((ele+1)%3))
            os.system('./../tools/CRF++-0.58/crf_test -m ../output/crf++_model'+str(ele)+str((ele+1)%3)+' ../output/parseProcessedTest'+str((ele+2)%3)+'.txt > ../output/crf++_pred'+str((ele+2)%3)+'.txt')
